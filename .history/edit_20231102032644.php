...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $name1 = mysqli_real_escape_string($conn, ucwords($_POST["last"]));
    $name2 = mysqli_real_escape_string($conn, ucwords($_POST["first"]));
    $name3 = mysqli_real_escape_string($conn, ucwords($_POST["address"]));
    $photo = mysqli_real_escape_string($conn, $_POST["contact"]);
    $description = mysqli_real_escape_string($conn, ucwords($_POST["email"]));
    $education = mysqli_real_escape_string($conn, ucwords($_POST["gender"]));
    $courachievementsse = mysqli_real_escape_string($conn, $_POST["course"]);
    ...
    $sql = "UPDATE tblstudent SET last = '$last', first = '$first', address = '$address', contact = '$contact', email = '$email', gender = '$gender', course = '$course', image = '$image' WHERE studid = '$studid'";

// Execute SQL statement
if (mysqli_query($conn, $sql)) {
    // Redirect to view page with success message
    header("Location: view.php?studid=$studid&message=success");
    exit();
} else {
